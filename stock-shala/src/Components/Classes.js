...
export default function Classes() {
  ...
  return (
    <div className="p-8 bg-gray-100 min-h-screen">
      <h1 className="text-3xl font-bold mb-6 text-center">Live Classes</h1>
     
{/* profile ======================> */}

<div
      className="relative inline-block flex flex-row-reverse"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      {/* Profile Icon */}
        
      <div className="bg-gray-500 p-1 rounded-full cursor-pointer mb-4 mr-4">
        <img
      ...
      {isHovered && (
        <div className="absolute right-0 mt-10 w-32 bg-white rounded-lg shadow-lg p-2">
          <button
            className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-1 rounded-full w-full"
...
{/* profile ======================> */}
      {loading && <p className="text-green-500 text-center text-3xl">Loading classes...</p>}
      {error && <p className="text-red-500">{error}</p>}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 p-3 mt-6">
        ...
        classes.map((cls, index) => (
          ...
          >
            <img
              src={cls.image}
              alt={cls.category.title}
              className="w-full h-40 object-cover rounded mb-4"
            />
            <h2 className="text-xl font-bold">{cls.category.title}</h2>
            
            <p className="text-blue-500">{cls.teachers.name}</p>
            <p className="text-gray-500">{cls.time}</p>
            <p className="text-gray-600 text-xs mt-1">{new Date(cls.start_datetime).toLocaleString()}</p>
